...
set_auto_logout() {
    echo "Setting automatic logout for all users after 60 minutes of inactivity..."
    ...
    if ! grep -q "TMOUT=" /etc/profile; then
        ...
        sed -i '/TMOUT=/c\TMOUT=3600' /etc/profile
    fi
}
...
# Function to configure timezones
configure_timezone() {
   echo "Comfigure timezones..."
